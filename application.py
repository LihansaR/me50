...
def index():
    ...
    stocks = db.execute("SELECT symbol, shares FROM portfolios WHERE user_id = :user_id ORDER BY symbol DESC", user_id = session["user_id"])
...
def buy():
    ...
    if request.method == "POST":
        ...
        db.execute("UPDATE useres SET cash = :balance WHERE id = :user_id", user_id = session["user_id"], balance = user_balance)

         portfolio = db.execute("SELECT shares FROM portfolios WHERE user_id = :user_id AND symbol = :symbol", user_id = session["user_id"], symbol = input_symbol)

        if len(portfolio) == 1:

            shares = portfolio[0]["shares"] + int(input_shares)

            db.execute("UPDATE portfolios SET shares = :shares WHERE user_id = :user_id and symbol = :symbol", user_id = session["user_id"], symbol = input_symvol, shares = shares)

        else:
            db.execute("INSERT INTO portfolios (user_id, symbol, shares) VALUE (:user_id, :symbol, :shares)", user_id = session["user_id"], symbol = input_symbol, shares = int(input_shares))
...
def history():
    ...
    transactions = db.execute("SELECT date, type, symbol, shares, price FROM transactions WHERE user_id = :user_id ORDER BY date DESC", user_id = session["user_id"])
...
def register():
    ...
    if request.method == "POST":
        ...
        username = db.execute("SELECT username FROM users WHERE username = :username", username = input_username)

        if len(username) == 1:
            return apology("sorry, username is already taken", 403)

        else:
            new_user = db.execute("INSERT INTO users (username, hash) VALUES (:username, :password)", username = input_username, password = generate_password_hash(input_password, method="pbkdf2:sha256", salt_length=8))
...
def sell():
    ...
    if request.method == "POST":
        ...
        stocks = db.execute("SELECT shares FROM portfolios WHERE user_id = :user_id AND symbol = :symbol", user_id = session["user_id"], symbol = input_symbol)
        ...
        db.execute("INSERT INTO transactions (user_id, type, symbol, shares, price) VALUES (:user_id, :transaction_type, :symbol, :shares, :price)", user_id = session["user_id"], transaction_type = "sell", symbol = input_symbol, shares = int(input_shares), price = format(price,".2f"))
        ...
        db.execute("UPDATE portfolios SET shares = :shares WHERE user_id = :user_id AND symbol = :symbol", user_id = session["user_id"], symbol = input_symbol, shares = shares)
...
def wallet():
    ...
    if request.method == "POST":
        ...
        db.execute("UPDATE users SET cash = cash + :amount WHERE id = :user_id", user_id = session["user_id"], amount = input_amount)
